...
const MAPPING_KEYS: (keyof LabelMappingDto)[] = [
  "classRegistration",
  "task",
  "inquiry",
  "other",
        ...
        <div className="flex flex-col gap-3">
          {MAPPING_KEYS.map((key) => {
            ...
            return (
              <div
                key={key}
                className="flex items-center gap-3 rounded-xl border border-gray-100 px-4 py-2.5 dark:border-white/10"
              >
                ...
                {editing ? (
                  <select
                    ...
                    className="dark:bg-navy-700 focus:border-brand-500 flex-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm text-gray-700 outline-none dark:border-white/10 dark:text-white"
                  >
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    {gmailLabels.find((g) => g.value === currentValue)?.label ??
                      currentValue ?? <span className="italic">Chưa gán</span>}
                  </span>
                )}
              </div>
            );
