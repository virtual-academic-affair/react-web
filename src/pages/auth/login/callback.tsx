...
export default function GoogleCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!code) {
      redirectToLoginWithError("Thiếu mã xác thực. Vui lòng đăng nhập lại.");
      return;
    }
    ...
    const authenticate = async () => {
      try {
        ...
        const isBanned = (err as { statusCode?: number })?.statusCode === 403;

        if (isBanned) {
          navigate("/auth/banned", { replace: true, state: { isBannedFlow: true } });
          return;
        }

        redirectToLoginWithError("Đăng nhập thất bại. Vui lòng thử lại.");
      }
    };
