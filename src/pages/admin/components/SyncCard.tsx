...
const SyncCard: React.FC<SyncCardProps> = ({
  ...
  return (
    <div className="font-dm shadow-3xl shadow-shadow-500 dark:bg-navy-800! rounded-primary grid w-full grid-cols-1 gap-3 bg-white bg-clip-border p-3 lg:grid-cols-11 dark:shadow-none">
      {/* Left side - Icon */}
      <div className="col-span-5 h-full w-full rounded-xl p-3 lg:col-span-5 lg:pb-0">
        <div className="flex h-full w-full flex-col items-center justify-center rounded-xl py-3">
          <div className="text-brand-500 bg-lightPrimary dark:bg-navy-700! mt-2 flex items-center justify-center rounded-full p-6.5 text-5xl font-bold dark:text-white">
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth="0"
      ...
      <div className="dark:bg-navy-800! col-span-5 flex h-full w-full flex-col justify-center overflow-hidden rounded-xl bg-white pl-3 lg:col-span-6">
        ...
        <div className="mb-4 list-inside list-disc text-base text-gray-600 dark:text-gray-400">
          ...
          gán nhãn email thông minh, đồng thời xử lý theo nghiệp vụ nhanh chóng.
        </div>
        ...
        <button
          onClick={handleSync}
          disabled={syncing}
          className="linear bg-brand-500 hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 dark:hover:bg-brand-300 dark:active:bg-brand-200 mt-2 flex items-center justify-center rounded-xl px-2 py-2 text-base font-medium text-white transition duration-200 disabled:opacity-60 dark:text-white"
        >
