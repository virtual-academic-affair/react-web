...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  return (
    <StandardModal
      ...
      confirmColor={danger ? "bg-red-500 hover:bg-red-600" : "bg-brand-500 hover:bg-brand-600"}
      confirmIcon={icon === "delete" ? <MdDelete className="h-4 w-4" /> : undefined}
      loading={loading}
    >
        ...
        {subTitle && (
          <div className={`flex gap-4 rounded-2xl p-4 ${danger ? "bg-red-50 dark:bg-red-500/10" : "bg-brand-50 dark:bg-brand-500/10"}`}>
            ...
            <div className="flex flex-col gap-1">
              <p className={`text-base font-bold ${danger ? "text-red-800 dark:text-red-200" : "text-brand-800 dark:text-brand-200"}`}>
